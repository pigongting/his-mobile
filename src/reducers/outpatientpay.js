...
import * as fetch from '../services/outpatientpay';
import { changeDataType } from '../utils/handleData';

/* 更新 */
export function *fetchUpdateRow(action, { call, put, select }, namespace) {
  const { data } = yield call(fetch.updateRow, { errormsg: '门诊缴费详情查询失败', ...action }, {}, {
...
export function *fetchViewedRow(action, { call, put, select }, namespace) {
  const { data } = yield call(fetch.viewedRow, { errormsg: '门诊缴费详情查询失败', ...action }, {}, {
    orderId: action.payload,
  });
  if (data.orderStatus === 0) {
    yield put({ type: 'fetchDetailList', payload: data.orderNo });
  }
...
export function *fetchAllData(action, { call, put, select }, namespace) {
  const { data } = yield call(fetch.listAllData, { errormsg: '门诊缴费记录查询失败', ...action }, {}, {
...
export function *fetchDetailList(action, { call, put, select }, namespace) {
  const { data } = yield call(fetch.listDetailList, { errormsg: '门诊缴费明细查询失败', ...action }, {}, {
