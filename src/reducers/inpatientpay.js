...
import * as fetch from '../services/inpatientpay';
import { changeDataType } from '../utils/handleData';

/* 查看 */
export function *fetchViewedRow(action, { call, put, select }, namespace) {
  const { data } = yield call(fetch.viewedRow, { errormsg: '住院缴费详情查询失败', ...action }, {}, {
...
export function *fetchAllData(action, { call, put, select }, namespace) {
  const { data } = yield call(fetch.listAllData, { errormsg: '住院缴费记录查询失败', ...action }, {}, {
