...
export function *fetchViewedRow(action, { call, put, select }, namespace) {
  const { data } = yield call(
    (atp, config, options) => request(atp, config, { method: 'POST', body: options, Url: iface.getReportDetailById }),
    { errormsg: '报告详情查询失败', ...action }, {}, { reportId: action.payload },
  );
...
export function *fetchAllData(action, { call, put, select }, namespace) {
  const { data } = yield call(
    (atp, config, options) => request(atp, config, { method: 'POST', body: options, Url: iface.getReportList }),
    { errormsg: '报告记录查询失败', ...action }, {}, { visitNo: action.payload },
  );
